...
class Index extends Component {
  ...
  listaDedocumentosDoCredenciamento = async (id_credenciamento) => {
    try {
      const documentos = await listaDedocumentosDoCredenciamentoApi(id_credenciamento);
      this.setState({ arrayDocumentosDoCredenciamento: documentos });
    } catch (error) {
      console.error('Erro ao listar documentos de credenciamento:', error);
    }
  };
              ...
              <ul>
                {arrayDocumentosDoCredenciamento.length > 0 ? (
                  arrayDocumentosDoCredenciamento.map((documento, index) => (
                    <li key={index}>
                      <a href={documento.anexo} target="_blank" rel="noopener noreferrer">
                        Ver Anexo
                      </a>
