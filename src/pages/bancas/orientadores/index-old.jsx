...
export default class Index extends Component {
	...
	listaDeOrientandos = async (token, idOrientador, nome = "", idLinhaPesquisa = 0, idFaseProcesso = 0) => {
		try {
			...
			// Verifica se a resposta é ok antes de tentar manipular os dados
			if (!response.ok) {
			...
			if (data.status === 200 && data.resultados.length > 0) {
				...
				console.error('Nenhum resultado encontrado ou erro na API:', data);
			}
		} catch (error) {
			console.error('Erro ao tentar buscar orientandos:', error);
		}
	};
	...
	listaDeBancas = async (token) => {
		try {
			// Fazer a requisição para bancas de qualificação (id_tipoBanca = 1)
			const responseQualificacao = await fetch(`${api.baseURL}/orientadores/${token}/bancas?tipo_banca=1`, {
				method: 'GET',
				headers: {
					Accept: 'application/json',
					'Content-Type': 'application/json',
			...
			// Fazer a requisição para bancas de defesa (id_tipoBanca = 2)
			const responseDefesa = await fetch(`${api.baseURL}/orientadores/${token}/bancas?tipo_banca=2`, {
				method: 'GET',
				headers: {
					Accept: 'application/json',
					'Content-Type': 'application/json',
			...
			if (!responseQualificacao.ok || !responseDefesa.ok) {
				throw new Error(`Erro nas requisições: Qualificação - ${responseQualificacao.status}, Defesa - ${responseDefesa.status}`);
			}

			// Processa os dados de qualificação e defesa
			const dataQualificacao = await responseQualificacao.json();
			const dataDefesa = await responseDefesa.json();

			console.log('Qualificação e Defesa:', dataQualificacao, dataDefesa);

			if (dataQualificacao.status === 200 && dataDefesa.status === 200) {
				this.setState({
					array_bancasQ: dataQualificacao.resultados,
					array_bancasD: dataDefesa.resultados,
				});
			} else {
				console.error('Erro nos dados recebidos de qualificação ou defesa:', dataQualificacao, dataDefesa);
			}
		} catch (error) {
			console.error('Erro ao tentar buscar bancas:', error);
		}
	};
	...
	cadastrarATA = async (e) => {
		...
		try {
			...
			if (data.status === 200) {
				this.setState({ success: data.msg });
				// Recarregar as bancas para ambos os tipos após a operação
				this.listaDeBancas(getToken());
			}
	...
	atualizarATA = async (e) => {
		...
		try {
			...
			if (data.status === 200) {
				this.setState({ success: data.msg });
				this.listaDeBancas(getToken()); // Recarregar as bancas após atualizar a ATA
			}
	...
	cadastrarFichaDeAvaliacao = async (e) => {
		...
		try {
			...
			if (!titulo_projeto || !pergunta_condutora || !hipotese || !fundamentacao_teorica || !objetivo || !metodo || !cronograma || !conclusao_avaliacao) {
				this.setState({ error: 'Por favor, preencher todos os campos!' });
				return;
			}
			...
			if (data.status === 200) {
				this.setState({ success: data.msg });
				this.listaDeBancas(getToken()); // Recarregar as bancas após cadastrar a ficha de avaliação
			}
																...
																bancasQ.map(banca => (
																	<Accordion defaultActiveKey="0" flush style={{ backgroundColor: banca.status_confirmacaoBancaD === "FINALIZADA" ? "#00ff87" : "" }}>
																		<Accordion.Item eventKey={banca.id} style={{ backgroundColor: '#fffffff', marginBottom: '5px' }}>
																			<Accordion.Header>
																				<h5><FaLayerGroup /> {banca?.orientando?.toLocaleUpperCase() || ""} - {banca.status_confirmacaoBancaD}</h5>
																			...
																			<Accordion.Body style={{ overflowY: "scroll", height: "350px" }}>
																				<div className='d-flex flex-column'>
																					...
																					{banca.status_confirmacaoBancaQ === "CONFIRMADO" && banca.id_ata === null ? (<button className='button mt-2 ' onClick={() => this.handlerShowModalEmitirAta(banca)}>Emitir ATA</button>) : ""}
																					{banca.id_ata !== null ? (<button className='button mt-2' onClick={() => this.handlerShowModalAtualizarAta(banca)}>Atualizar ATA</button>) : ""}
																					{banca.id_ata !== null ? (<button className='button mt-2 ' onClick={() => this.handlerShowModalVisualizarAta(banca)}>ATA</button>) : ""}
																					{banca.id_fichaAvaliacao === null ? (<button className='button mt-2' onClick={() => this.handlerShowModalEmitirFichaDeAvaliacao(banca)}>Emitir ficha de avaliação</button>) : ""}
																					{banca.id_fichaAvaliacao !== null ? (<button className='button mt-2' onClick={() => this.handlerShowModalEditarFichaDeAvaliacao(banca)}>Atualizar ficha de avaliação</button>) : ""}
																					{banca.id_fichaAvaliacao !== null ? (<button className='button mt-2' onClick={() => this.handlerShowModalVisualizarFichaDeAvaliacao(banca)}>Visualizar ficha de avaliação</button>) : ""}
																		...
																		<Accordion.Item eventKey={banca.id} style={{ backgroundColor: '#fffffff', marginBottom: '5px' }}>
																			...
																			<Accordion.Body style={{ overflowY: "scroll", height: "350px" }}>
																				<div className='d-flex flex-column'>
																					...
																					{banca.status_confirmacaoBancaD === "CONFIRMADO" && banca.id_ata === null ? (<button className='button mt-2' onClick={() => this.handlerShowModalEmitirAta(banca)}>Emitir ATA</button>) : ""}
																					{banca.id_ata !== null ? (<button className='button mt-2' onClick={() => this.handlerShowModalAtualizarAta(banca)}>Atualizar ATA</button>) : ""}
																					{banca.id_ata !== null ? (<button className='button mt-2 ' onClick={() => this.handlerShowModalVisualizarAta(banca)}>ATA</button>) : ""}
																					{banca.id_fichaAvaliacao === null ? (<button className='button mt-2' onClick={() => this.handlerShowModalEmitirFichaDeAvaliacao(banca)}>Emitir ficha de avaliação</button>) : ""}
																					{banca.id_fichaAvaliacao !== null ? (<button className='button mt-2' onClick={() => this.handlerShowModalEditarFichaDeAvaliacao(banca)}>Atualizar ficha de avaliação</button>) : ""}
